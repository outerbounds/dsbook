...
def install_tf():
    # FIXME: remove these lines once tensorflow==2.6.0 becomes availabe in Conda
    import sys,subprocess
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', '-q', 'tensorflow==2.6.0'])
...
class FeatureEncoder():
    ...
    def _coords_to_grid(cls, table):
        ...
        for i in range(len(plat)):
            pcode = geohash.encode(plat[i], plon[i], precision=6)
            dcode = geohash.encode(dlat[i], dlon[i], precision=6)
